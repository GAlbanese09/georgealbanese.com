...
New hire account provisioning was entirely manual. When HR submitted a Jira onboarding ticket, an IT team member would read the ticket, manually create an Azure AD account in the Entra portal, assign an M365 license, type up a welcome email with temporary credentials, and send it from their personal mailbox. The process took 15–30 minutes per hire, was error-prone, and depended on someone being available to do it.

An early-stage data sync tool existed that could ingest employee records from the HRIS into a SQL database and push attributes to Azure AD and Exchange — but it couldn't provision new accounts. It was a reconciliation engine, not an onboarding pipeline.

## What I Built

I designed and built a three-system automated onboarding pipeline over Q1 2026 that handles the full employee lifecycle from Jira ticket to fully provisioned identity — with zero human intervention for standard onboardings.
...
PowerShell 7.4, Microsoft Graph API, Azure Automation Runbooks, Azure SQL, Power Automate, Jira REST API, Exchange Online, Entra ID, Office Message Encryption
